module CustomerioHelper
	...
	def cio_user_multiple_locations(user)
		...
		$customerio.identify(
			...
			locations_last_added_at: format_as_unix_timestamp(user.bus_vendor.locations.where(:active => true).order('created_at DESC').first.created_at),
			locations_last_updated_at: format_as_unix_timestamp(user.bus_vendor.locations.where(:active => true).order('updated_at DESC').first.updated_at),
